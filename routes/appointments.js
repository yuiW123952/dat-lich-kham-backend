...
router.put('/:id/checkin', async (req, res) => {
  try {
    const { collect_cash } = req.body; // true nếu thu tiền mặt
    const [[appt]] = await db.query('SELECT id, status, payment_status FROM appointments WHERE id=?', [req.params.id]);
    if (!appt) return res.json({ success: false, message: 'Không tìm thấy lịch hẹn' });
    if (appt.status === 'cancelled') return res.json({ success: false, message: 'Lịch đã bị hủy' });

    if (collect_cash) {
      // Thu tiền mặt + check-in
      await db.query(`UPDATE appointments SET checked_in=1, payment_status='paid', checked_in_at=NOW(), status='waiting' WHERE id=?`, [req.params.id]);
    } else {
...
// PUT /api/appointments/:id/test-orders/collect-cash - lễ tân thu tiền mặt xét nghiệm
router.put('/:id/test-orders/collect-cash', async (req, res) => {
  try {
    const [result] = await db.query(`
      UPDATE test_orders SET status='paid', payment_method='cash', paid_at=NOW()
      WHERE appointment_id=? AND status='pending'`, [req.params.id]);
    if (result.affectedRows === 0)
      return res.json({ success: false, message: 'Không có xét nghiệm nào cần thanh toán' });
    res.json({ success: true });
  } catch (e) { res.json({ success: false, message: 'Lỗi server' }); }
});

// GET /api/appointments/test-orders-by-cccd - lễ tân tìm xét nghiệm theo CCCD
router.get('/test-orders-by-cccd', async (req, res) => {
  try {
    const { cccd, date } = req.query;
    if (!cccd) return res.json({ success: false, message: 'Thiếu CCCD' });
    const dateFilter = date || new Date().toISOString().slice(0, 10);

    const [rows] = await db.query(`
      SELECT a.id AS appointment_id, pp.full_name AS patient_name,
             pp.date_of_birth, s.date, dep.name AS department_name,
      ...
      JOIN departments dep ON s.department_id = dep.id
      WHERE pp.cccd = ? AND s.date = ?
      AND EXISTS (
        SELECT 1 FROM test_orders t WHERE t.appointment_id = a.id AND t.status = 'pending'
      )`, [cccd, dateFilter]);

    // Load test_orders cho từng appointment
    for (const row of rows) {
      const [orders] = await db.query(`
        SELECT to2.*, tt.name AS test_name, tt.price
        FROM test_orders to2
        JOIN test_types tt ON to2.test_type_id = tt.id
